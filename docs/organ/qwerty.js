 ...
 */
var BASE = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11 }, KEYS = {};
!function() { for (var E = 0; E <= 10; ++E)for (var r in BASE) { var S = BASE[r] + (12 * E); KEYS[S] = r + E, KEYS[S + 1] = r + "#" + E } }();
(function() {
  var root = this;
  ...
  */
  ...
  var createKeys = function() {
    ...
    for (i = 0; i < total_white_keys; i++) {
      ...
      key = createKey({
        ...
        id: Object.values(KEYS).findIndex(e => e === this.whiteNotes[note_counter] + octave_counter),
        noteNumber: i
      });
        ...
        this.notesWithSharps.forEach(function(note, index) {
          if (note === that.whiteNotes[note_counter]) {
            key = createKey({
              ...
              id: Object.values(KEYS).findIndex(e => e === that.whiteNotes[note_counter] + '#' + octave_counter),
              noteNumber: i
            });
  ...
  var getKeyPressed = function(keyCode) {
    return parseInt(key_map[keyCode] + (12*parseInt(settings.keyOctave, 10))+ settings.keyPressOffset)+8;
  ...
  var setKeyDown = function(key, callback, u) {
    callback(key, getFrequencyOfNote(key), u);
    lightenUp(document.getElementById(key));
  };
  var setKeyUp = function(key, callback, u) {
    callback(key, getFrequencyOfNote(key), u);
    darkenDown(document.getElementById(key));
  };
  /**
  ...
  var QwertyHancock = function(settings) {
    ...
    this.pressKey = function(midi) {
      setKeyDown(midi, this.keyDown, true);
    }
    this.pressKeyUp = function(midi) {
      u = true;
      setKeyUp(midi, this.keyUp, true);
    }
